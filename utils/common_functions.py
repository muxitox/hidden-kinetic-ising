...
def plot_likelihood_MSE(ell_list, error_list, eta, error_iter_list, MSE_m_list, MSE_C_list, MSE_D_list,
    ...
    ax[0].plot(ell_list, label='log(Likelihood)')
    ax[0].plot(np.square(error_list), label='(max_gradient)^2')
    ax[0].plot(np.diff(ell_list / eta), '--', label='np.diff(log(Likelihood))/eta')
    ax[0].legend()
